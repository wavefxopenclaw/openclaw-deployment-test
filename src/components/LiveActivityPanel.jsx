            ...
            agents.map((agent) => (
              <div key={agent.id} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs tabular-nums text-slate-300">${agent.estimatedCost?.toFixed?.(2) ?? agent.estimatedCost}</span>
                </div>
              </div>
            ))
          ) : (
